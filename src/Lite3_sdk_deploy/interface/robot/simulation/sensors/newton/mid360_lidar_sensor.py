...
class NewtonMid360LidarSensor:
    ...
    def init_visuals(builder):
        ...
        mount_shape = find_builder_shape_index(builder, MID360_MOUNT_SITE_NAME)
    ...
    def __init__(self, model, node, enabled: bool = False, pattern_path=None, samples_per_scan=MID360_SAMPLES_PER_SCAN, downsample=1):
        self.model = model
        self.node = node
        ...
        self.site_index = find_site_index(model, MID360_MOUNT_SITE_NAME)
        ...
        path = Path(pattern_path) if pattern_path is not None else Path(__file__).resolve().parents[1] / MID360_PATTERN_FILE
        ...
        node.get_logger().info(
            f"[INFO] Newton Mid360 initialized ({self.ray_count} rays @ {MID360_FREQUENCY_HZ} Hz, pattern: {path})"
        )
    ...
    def update(self, state, timestamp: float):
        ...
        valid = (ranges >= MID360_RANGE_MIN) & (ranges <= MID360_RANGE_MAX)
        ...
        self.pub.publish(make_xyz_pointcloud(points, self.node.get_clock().now().to_msg(), MID360_FRAME_ID))
    ...
    def get_static_transforms(self, stamp):
        ...
        return [make_transform(stamp, "base_link", MID360_FRAME_ID, site_pos_local, quat_from_matrix(site_rot))]
