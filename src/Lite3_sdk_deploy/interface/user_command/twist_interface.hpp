...
private:
    ...
    void TwistCallback(const geometry_msgs::msg::Twist::SharedPtr msg) {
        ...
        // Pass velocity values directly (same as keyboard interface),
        // clamped to the training range. No normalization to [-1,1].
        usr_cmd_->forward_vel_scale  = std::clamp(static_cast<float>(msg->linear.x),
                                                   -max_linear_x_, max_linear_x_);
        usr_cmd_->side_vel_scale     = std::clamp(static_cast<float>(msg->linear.y),
                                                   -max_linear_y_, max_linear_y_);
        // Pass velocity directly, matching keyboard interface convention.
        usr_cmd_->turnning_vel_scale = std::clamp(static_cast<float>(msg->angular.z),
...
public:
        ...
        : UserCommandInterface(robot_name), node_(node)
    {
        ...
        RCLCPP_INFO(node_->get_logger(), "TwistInterface: subscribed to /cmd_vel");
    }
