...
Adapted for M20 (base body is 'base_link' rather than 'TORSO').
"""
...
class DepthSensor:
    ...
    def __init__(
        ...
        if self.enable_depth:
            self._depth_info = make_camera_info(
                self.config.depth_fx, self.config.depth_fy,
                self.config.depth_cx, self.config.depth_cy,
                self.config.width, self.config.height,
                self.config.depth_optical_frame,
            )
        if self.enable_color:
            self._color_info = make_camera_info(
                self.config.color_fx, self.config.color_fy,
                self.config.color_cx, self.config.color_cy,
                self.config.width, self.config.height,
    ...
    def get_static_transforms(self, stamp):
        ...
        depth_pos_local = base_rot.T @ (depth_pos_w - base_pos)
        depth_rot_local = base_rot.T @ depth_rot_w
        ...
        if self.color_cam_id >= 0:
            ...
            color_pos_local = base_rot.T @ (color_pos_w - base_pos)
