"""M20 simulation configuration dataclass with YAML loading and overrides."""
...
DEFAULT_SCENE_URI = "package://m20_sdk_deploy/M20_description/m20_mjcf/mjcf/M20_stair_scene.xml"
DEFAULT_ROBOT_DESCRIPTION_URI = "package://m20_sdk_deploy/M20_description/m20_mjcf/mjcf/M20.xml"
...
class SimulationConfig:
    ...
    def validate(self) -> list[str]:
        ...
        if simulator != "mujoco":
            errors.append("simulator must be 'mujoco' for M20")
        ...
        if robot_path.suffix.lower() not in {".xml", ".mjcf"}:
            errors.append("robot_description must be an MJCF/XML file")
...
def candidate_package_roots() -> list[Path]:
    ...
    for parent in Path(__file__).resolve().parents:
        ...
        for candidate in candidates:
            ...
            if resolved in seen:
                continue
            if (resolved / "M20_description").exists() or (resolved / "config").exists():
...
def resolve_path(path_value: str | os.PathLike[str], must_exist: bool = False) -> Path:
    ...
    for root in candidate_package_roots():
        for base in (
            root,
            root / "M20_description" / "m20_mjcf" / "mjcf",
            root / "config",
        ):
...
def _dataclass_to_dict(value):
    if is_dataclass(value):
        return {fi.name: _dataclass_to_dict(getattr(value, fi.name)) for fi in fields(value)}
